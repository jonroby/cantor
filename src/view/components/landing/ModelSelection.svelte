		...
		<div class="card">
			...
			{#if activeTab === 0}
				<div class="provider-grid">
					{#each frontierProviders as p (p.id)}
				...
				<div class="single-card-wrap">
					<div class="provider-card single-card">
						...
						<div class="ollama-model-list">
							{#each filteredOllama as model (model)}
				...
				<div class="single-card-wrap">
					<div class="provider-card single-card">
						...
						<div class="context-row">
							...
							{#each contextOptions as opt (opt)}
								<button
									class="context-btn"
									class:context-btn-active={webllmContext === opt}
									onclick={() => (webllmContext = opt)}>{opt}</button
								>
							{/each}
						</div>
						<input
						...
						<div class="webllm-grid">
							{#each filteredWebllm as m (m.name)}
