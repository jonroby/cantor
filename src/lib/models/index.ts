...
const KEY_BASED_PROVIDER_DEFINITIONS: Record<KeyBasedProvider, KeyBasedProviderDefinition> = {
	claude: {
		config: {
			name: 'Claude',
...
) as Record<string, KeyBasedProvider>;
...
) as Record<KeyBasedProvider, Record<string, number>>;
...
export function getModelContextLength(provider: Provider, modelId: string): number | null {
	if (!isKeyBasedProvider(provider)) {
		return null;
	}
